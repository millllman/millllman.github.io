...
(function($) {
	...
	init = function(el){
		...
		imgInt[el.id] = setInterval(function() { $.transition(el)  
	...
	$.transition = function(el,direction){
		...
		if((opts[el.id].direction == 'right' && order[el.id][0] == 1) 
			...
			|| opts[el.id].direction == 'fountainAlternate')			
				...
				//var id = $(el).css('background-image').replace(/[^\d]+/g, '');
				//var id = $('#slideshowHolder').css('background-image').replace(/[^\d]+/g, '');
				
				//console.log(id)
				//id = parseInt(id);
				//id = (id < 6) ? (++id) : 1;
				//$('.preview .active').removeClass('active');
				//$('#p_0'+id).addClass('active')
